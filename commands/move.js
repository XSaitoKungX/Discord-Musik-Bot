...
module.exports = {
  name: "move",
  description: "Verschiebt einen Track an eine angegebene Position.",
  usage: "",
  permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!args[0] || !args[1])
      return client.sendTime(message.channel, "❌ | **Ungültige Argumente.**");

    // Check if (args[0] - 1) is a valid index
    let trackNum = parseInt(args[0] - 1);
    if (trackNum < 1 || trackNum > player.queue.length - 1) {
      return client.sendTime(message.channel, "❌ | **Ungültige Titelnummer.**");
    }
  ...
  SlashCommand: {
    options: [
      {
        ...
        type: 4,
        required: true,
        description: "Verfolgen, um sich zu bewegen.",
      },
      {
        name: "position",
        value: "track2",
        type: 4,
        required: true,
        description: "Verschiebt den ausgewählten Titel an die angegebene Position.",
      },
    ],
    /**
     *
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (!args[0].value || !args[1].value)
        return client.sendTime(interaction, "❌ | **Ungültige Titelnummer.**");
