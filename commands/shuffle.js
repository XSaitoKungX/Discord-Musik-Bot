...
module.exports = {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!player.queue || !player.queue.length || player.queue.length === 0)
      return client.sendTime(
        message.channel,
        "❌ | **Nicht genug Songs in der Warteschlange zum Mischen!**"
      );
    player.queue.shuffle();
    await client.sendTime(message.channel, "✅ | Warteschlange erfolgreich gemischt!");
  },
  SlashCommand: {
    /**
     *
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      player.queue.shuffle();
      client.sendTime(interaction, "✅ | Warteschlange gemischt!");
    },
  },
};
