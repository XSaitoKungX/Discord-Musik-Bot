...
module.exports = {
  name: "search",
  description: "Zeigt ein Ergebnis von Songs basierend auf der Suchanfrage an",
  usage: "[song]",
  permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (Searched.loadType == "NO_MATCHES")
      return client.sendTime(
        message.channel,
        "Keine Treffer gefunden für " + SearchString
      );
    else {
      ...
      let Pages = songs.map((songz) => {
        ...
        let em = new MessageEmbed()
          .setAuthor(
            "Suchergebnisse von " + SearchString,
      ...
      let msg = await message.channel.send(
        "**Gib die Nummer des Songs ein, den du abspielen möchtest! Läuft in `30 Sekunden` ab.**"
      );
      ...
      let SongID = await message.channel
        ...
        .catch(() => {
          er = true;
          msg.edit(
            "**Du hast zu lange gebraucht, um zu antworten. Führt den Befehl erneut aus, wenn du etwas spielen möchtest!**"
          );
        });
      if (er) return;
      /**@type {Message} */
      ...
      if (!parseInt(SongIDmsg.content))
        return client.sendTime(
          message.channel,
          "Bitte sende die richtige Song-ID-Nummer!"
        );
      let Song = Searched.tracks[parseInt(SongIDmsg.content) - 1];
      if (!Song)
        return client.sendTime(
          message.channel,
          "Kein Song für die angegebene ID gefunden!"
        );
    ...
    options: [
      {
        ...
        type: 3,
        required: true,
        description: "Gib den Songnamen oder die URL ein, die du suchen möchtest",
      },
    ],
    /**
     *
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (search.match(client.Lavasfy.spotifyPattern)) {
        ...
        switch (Searched.loadType) {
          case "LOAD_FAILED":
            ...
            return client.sendError(
              interaction,
              `:x: | **Bei der Suche ist ein Fehler aufgetreten!**`
            );
          ...
          case "NO_MATCHES":
            ...
            return client.sendTime(
              interaction,
              ":x: | **Es wurden keine Ergebnisse gefunden**"
            );
          case "TRACK_LOADED":
        ...
        try {
          ...
          return client.sendTime(
            interaction,
            `:x: | **Bei der Suche ist ein Fehler aufgetreten:** ${err.message}`
          );
        }
        switch (res.loadType) {
          case "NO_MATCHES":
          ...
          case "SEARCH_RESULT":
            ...
            try {
              ...
              return awaitchannel.send(
                "❌ | **Du hast keine Auswahl angegeben!**"
              );
            }
            ...
            if (index < 0 || index > max - 1)
              return awaitchannel.send(
                `Die von dir angegebene Zahl war größer oder kleiner als die Suchsumme. Verwendungszweck - \`(1-${max})\``
              );
