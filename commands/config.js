...
module.exports = {
  name: "config",
  description: "Die Bot-Einstellungen bearbeiten",
  usage: "",
  permissions: {
...
`);
    ...
    let emoji = await ConfigMessage.awaitReactions(
      ...
      client.sendTime(
        message.channel,
        "❌ | **Du hast leider zu lange gebraucht, um zu antworten. Wenn du die Einstellungen bearbeiten möchtest, führt bitte den Befehl erneut aus!**"
      );
    ...
    if (em._emoji.name === "1️⃣") {
      await client.sendTime(
        message.channel,
        "In was möchtest du das Präfix ändern?"
      );
      ...
      if (!prefix.first())
        return client.sendTime(
          message.channel,
          "Du hast zu lange gebraucht, um zu antworten."
        );
      prefix = prefix.first();
      ...
      client.sendTime(
        message.channel,
        `Server Präfix erfolgreich gespeichert als \`${prefix}\``
      );
    } else {
      await client.sendTime(
        message.channel,
        "Bitte erwähnt die Rolle, die `DJs` haben sollen."
      );
      ...
      client.sendTime(
        message.channel,
        "DJ-Rolle erfolgreich gespeichert als <@&" + role.id + ">"
      );
    }
  },
  ...
  SlashCommand: {
    options: [
      {
        name: "prefix",
        description: "Das Präfix des Bots überprüfen",
        type: 1,
        required: false,
        options: [
          {
            name: "symbol",
            description: "Präfix des Bots festlegen",
            type: 3,
            required: false,
          },
        ],
      ...
      {
        name: "dj",
        description: "Die DJ-Rolle überprüfen",
        type: 1,
        required: false,
        options: [
          {
            name: "role",
            description: "DJ-Rolle festlegen",
            type: 8,
            required: false,
          },
        ],
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (config === "prefix") {
        ...
        if (
          ...
          client.sendTime(
            interaction,
            `Das Präfix wurde jetzt auf \`${prefix}\` gesetzt`
          );
        } else {
          //has not prefix
          client.sendTime(
            interaction,
            `Das Präfix dieses Servers ist \`${GuildDB.prefix}\``
          );
        }
        ...
        if (
          ...
          client.sendTime(
            interaction,
            `Die DJ-Rolle dieses Servers wurde erfolgreich geändert in ${role.name}`
          );
        } else {
          /**
          ...
          client.sendTime(
            interaction,
            `Die DJ-Rolle dieses Servers ist ${role.name}`
          );
        }
      }
    },
